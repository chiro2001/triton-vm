...
where
    T: FromCode,
{
    let (aet, output_symbols) = parse_setup_simulate::<T>(
        code,
        input_symbols.clone(),
...
impl SourceCodeAndInput {
    ...
    pub fn run(&self) -> Vec<BFieldElement> {
        ...
        let (_, output, err) =
            simulate(Box::new(program), self.public_input(), self.secret_input());
        if let Some(e) = err {
